...
from constants import tags2int
from constants import NUM_ENTITIES
...
from constants import IGNORE_ALL
from constants import ACCEPT_ALL
from constants import PARALLEL_SIZE
from constants import EVAL_NUM

#int2tags = constants.int2tags

global NUM_QUERY_TYPES
global CONTEXT_TYPE
...
class Environment:
    def __init__(self, originalArticle, newArticles, goldEntities, indx,
        ...
        self.ignoreDuplicates = arguments.ignoreDuplicates
        self.entity = arguments.entity
        self.aggregate = arguments.aggregate
        self.delayedReward = arguments.delayedReward
        self.shooterLenientEval = arguments.shooterLenientEval
        self.listNum = 0  #start off with first list
        self.rlbasicEval = arguments.rlbasicEval
        self.rlqueryEval = arguments.rlqueryEval

        self.shuffledIndxs = [range(len(q)) for q in self.newArticles]
        if not evalMode and arguments.shuffleArticles:
        ...
        self.bestEntities = collections.defaultdict(
            lambda: '')  #current best entities
        ...
        if 0 in ENTITIES[self.indx][0]:
            self.prevEntities, self.prevConfidences = ENTITIES[self.indx][0][
                0], CONFIDENCES[self.indx][0][0]
        else:
            self.prevEntities, self.prevConfidences = self.extractEntitiesWithConfidences(
        ...
        self.allArticles = [originalArticle] + [
            item for sublist in self.newArticles for item in sublist
        ]
        ...
        if self.indx not in COSINE_SIM:
            ...
            for listNum, sublist in enumerate(self.newArticles):
                COSINE_SIM[self.indx][listNum] = cosine_similarity(
                    self.tfidf_matrix[0:1],
                    self.tfidf_matrix[cnt:cnt + len(sublist)])[0]
                pdb.set_trace()
                cnt += len(sublist)
    ...
    def extractEntitiesWithConfidences(self, article):
        ...
        pred, conf_scores, conf_cnts = predict.predictWithConfidences(
            trained_model, joined_article, False, helper.cities)
    ...
    def updateState(self, action, query, ignoreDuplicates=False):
        ...
        if ignoreDuplicates:
            nextArticle = None
            while not nextArticle and self.stepNum[listNum] < len(
                    self.newArticles[listNum]):
        ...
        if action != STOP_ACTION:
            ...
            for i in range(NUM_ENTITIES):
                if action != ACCEPT_ALL and i != action:
                    continue  #only perform update for the entity chosen by agent

                self.bestIndex = (self.prevListNum,
                                  self.prevArticleIndx)  #analysis
                if self.aggregate == 'majority':
                    self.bestEntitySet[i].append((entities[i], confidences[i]))
                    self.bestEntities[i], self.bestConfidences[
                        i] = self.majorityVote(self.bestEntitySet[i])
                else:
                    if i == 0:
                        ...
                        elif self.aggregate == 'always' or confidences[
                                i] > self.bestConfidences[i]:
                            self.bestEntities[i] = entities[
                                i]  #directly replace
                            # self.bestEntities[i] = self.bestEntities[i] + '|' + entities[i] #add to list
                            self.bestConfidences[i] = confidences[i]
                    else:
                        if not self.bestEntities[
                                i] or self.aggregate == 'always' or confidences[
                                    i] > self.bestConfidences[i]:
                            self.bestEntities[i] = entities[i]
                            self.bestConfidences[i] = confidences[i]
        ...
        if nextArticle and action != STOP_ACTION:
            ...
            if (articleIndx + 1) in ENTITIES[self.indx][listNum]:
                entities, confidences = ENTITIES[self.indx][listNum][
                    articleIndx +
                    1], CONFIDENCES[self.indx][listNum][articleIndx + 1]
            else:
                entities, confidences = self.extractEntitiesWithConfidences(
                    nextArticle)
                ENTITIES[self.indx][listNum][articleIndx + 1], CONFIDENCES[
                    self.indx][listNum][articleIndx +
                                        1] = entities, confidences
        ...
        if constants.mode == 'Shooter':
            matches = map(self.checkEquality,
                          self.bestEntities.values()[1:-1], entities[1:-1])
            matches.insert(0,
                           self.checkEqualityShooter(
                               self.bestEntities.values()[0], entities[0]))
            matches.append(
                self.checkEqualityCity(self.bestEntities.values()[-1],
                                       entities[-1]))
        else:
            matches = map(self.checkEqualityShooter,
                          self.bestEntities.values(), entities)
        ...
        for i in range(NUM_ENTITIES):
            ...
            self.state[NUM_ENTITIES +
                       i] = confidences[i]  #IMP: (original) next article state
        ...
        if nextArticle:
            ...
            self.state[4 * NUM_ENTITIES] = self.articleSim(self.indx, listNum,
                                                           articleIndx)
        else:
        ...
        if nextArticle and CONTEXT_TYPE != 0:
            ...
            for q in range(NUM_ENTITIES):
                if self.entity == NUM_ENTITIES or self.entity == q:
                    self.state[j:j + 2 * CONTEXT_LENGTH] = CONTEXT[self.indx][
                        listNum][articleIndx + 1][q]
                j += 2 * CONTEXT_LENGTH
    ...
    def checkEquality(self, e1, e2):
        ...
        return e2 != '' and (COUNT_ZERO or
                             e2 != 'zero') and e1.lower() == e2.lower()
    ...
    def calculateReward(self, oldEntities, newEntities):
        if constants.mode == 'Shooter':
            rewards = [
                int(self.checkEquality(newEntities[1], self.goldEntities[1])) -
                int(self.checkEquality(oldEntities[1], self.goldEntities[1])),
                int(self.checkEquality(newEntities[2], self.goldEntities[2])) -
                int(self.checkEquality(oldEntities[2], self.goldEntities[2]))
            ]
            ...
            for i in range(len(newEntities)):
                if self.goldEntities[i] != 'unknown':
                    rewards.append(
                        self.checkEqualityShooter(newEntities[i],
                                                  self.goldEntities[i]) -
                        self.checkEqualityShooter(oldEntities[i],
                                                  self.goldEntities[i]))
                else:
                    rewards.append(0.)
    ...
    def evaluateArticle(self, predEntities, goldEntities, shooterLenientEval,
                        shooterLastName, evalOutFile):
    ...
    def oracleEvaluate(self, goldEntities, entityDic, confDic):
        ...
        bestPred, bestCorrect = collections.defaultdict(
            lambda: 0.), collections.defaultdict(lambda: 0.)
                ...
                if constants.mode == 'Shooter':
                    ...
                    if goldEntities[0] != '':
                        ...
                        if correct > bestCorrect[int2tags[0]] or (
                                correct == bestCorrect[int2tags[0]] and
                                len(pred) < bestPred[int2tags[0]]):
                            # print "Correct: ", correct
                            # print "Gold:", gold
                            # print "pred:", pred
                            bestCorrect[int2tags[0]] = correct
                            bestPred[int2tags[0]] = 1 if len(pred) > 0 else 0.
                            bestConf[int2tags[0]] = confDic[stepNum][listNum][
                                0]
                        ...
                        if correct == 0:
                            EVALCONF2[int2tags[0]].append(
                                confDic[stepNum][listNum][0])

                    #all other tags
                    for i in range(1, NUM_ENTITIES):
                        if not COUNT_ZERO and goldEntities[
                                i].lower() == 'zero':
                            continue
                        gold = set(goldEntities[i].lower().split())
                        pred = set(predEntities[i].lower().split())
                        ...
                        if correct > bestCorrect[int2tags[i]] or (
                                correct == bestCorrect[int2tags[i]] and
                                len(pred) < bestPred[int2tags[i]]):
                            bestCorrect[int2tags[i]] = correct
                            bestPred[int2tags[i]] = len(pred)
                            bestConf[int2tags[i]] = confDic[stepNum][listNum][
                                i]
                        if stepNum == 0 and listNum == 0:
                            GOLD[int2tags[i]] += len(gold)
                        if correct == 0:
                            EVALCONF2[int2tags[i]].append(
                                confDic[stepNum][listNum][i])
                else:
                    #EMA
                    for i in range(NUM_ENTITIES):
    ...
    def thresholdEvaluate(self, goldEntities, arguments):
            ...
            for i in range(len(self.newArticles[listNum])):
                ...
                if sim > thres:
                    if (i + 1) in ENTITIES[self.indx][listNum]:
                        entities, confidences = ENTITIES[self.indx][listNum][
                            i + 1], CONFIDENCES[self.indx][listNum][i + 1]
                    else:
                        entities, confidences = self.extractEntitiesWithConfidences(
                            self.newArticles[listNum][i])
                    for j in range(NUM_ENTITIES):
        ...
        for i in range(NUM_ENTITIES):
            tmp = sorted(
                aggEntites[i].items(), key=itemgetter(1), reverse=True)
            bestEntities[i] = tmp[0][0]

            print i, tmp
            # pdb.set_trace()

        self.evaluateArticle(bestEntities, goldEntities,
                             arguments.shooterLenientEval,
                             arguments.shooterLastName, arguments.evalOutFile)

    #TODO for EMA
    def confEvaluate(self, goldEntities, arguments):
    ...
    def majorityVote(self, entityList):
        ...
        for entity, conf in entityList:
            ...
            confDic[entity] += conf
            if ticker == 0:
                dic[entity] += 0.1  #extra for original article to break ties
            ticker += 1

        bestEntity, bestVote = sorted(
            dic.items(), key=itemgetter(1), reverse=True)[0]
...
def baselineEval(articles, identifiers, arguments):
...
def thresholdEval(articles, downloaded_articles, identifiers, arguments):
    global CORRECT, GOLD, PRED
    CORRECT = collections.defaultdict(lambda: 0.)
    GOLD = collections.defaultdict(lambda: 0.)
    PRED = collections.defaultdict(lambda: 0.)
    for indx in range(len(articles)):
        ...
        newArticles = [[q.split(' ')[:WORD_LIMIT] for q in sublist]
                       for sublist in downloaded_articles[indx]]
        goldEntities = identifiers[indx]
        env = Environment(originalArticle, newArticles, goldEntities, indx,
                          arguments, False)
        env.thresholdEvaluate(env.goldEntities, arguments)
...
def confEval(articles, downloaded_articles, identifiers, arguments):
    global CORRECT, GOLD, PRED
    CORRECT = collections.defaultdict(lambda: 0.)
    GOLD = collections.defaultdict(lambda: 0.)
    PRED = collections.defaultdict(lambda: 0.)
    for indx in range(len(articles)):
        ...
        env = Environment(originalArticle, newArticles, goldEntities, indx,
                          arguments, False)
        env.confEvaluate(env.goldEntities, arguments)
    ...
    for indx, lists in ENTITIES.items():
        ...
        for listNum, articles in lists.items():
            for articleNum, entities in articles.items():
                ...
                if articleNum == 0:
                    #original article
                    article = [w.lower() for w in ARTICLES[indx][0]
                               ]  #need only the tokens, not tags
                else:
                    raw_article = DOWNLOADED_ARTICLES[indx][listNum][articleNum
                                                                     -
                                                                     1].lower()
                    cleaned_article = re.sub(r'[^\x00-\x7F]+', ' ',
                                             raw_article)
                ...
                for entityNum, entity in enumerate(entities):
                    ...
                    try:
                        assert (len(CONTEXT[indx][listNum][articleNum]) <=
                                NUM_ENTITIES)
                    except:
                        pdb.set_trace()
...
def top5_mean_std(bakra):
    bakra_sort = np.sort(bakra)
    chakra = bakra_sort[-5:]
    return np.mean(chakra), np.std(chakra)


def process(portNum, articleIdx, arguments):
    global train_articles, train_titles, train_identifiers, train_downloaded_articles
    global test_articles, test_titles, test_identifiers, test_downloaded_articles
    global articles, titles, identifiers, downloaded_articles
    global CONTEXT1, CONTEXT2
    ...
    while True:
        ...
        if message == "newGame":
            ...
            #print len(articleIdx)
            indx = articleIdx[articleNum % len(articles)]
            if DEBUG: print "INDX:", indx
            articleNum += 1
            originalArticle = articles[indx][
                0]  #since article has words and tags
        ...
        elif message == "evalInit":
            evalIterNum = 0
            evalStats = np.zeros(shape=(NUM_ENTITIES, 4,
                                        EVAL_NUM))  # (CORRECT,prec,rec,f1)
            ...
            articleNum = 0
            stepCnt = 0
            STAT_POSITIVE, STAT_NEGATIVE = [
                0 for i in range(NUM_ENTITIES)
            ], [0 for i in range(NUM_ENTITIES)]
            ...
            # Eval Calculation
            for tagNum in range(NUM_ENTITIES):
                tag = int2tags[tagNum]
                # Correct
                evalStats[tagNum][0][evalIterNum] = CORRECT[tag]
                # Precision
                prec = CORRECT[tag] / PRED[tag]
                evalStats[tagNum][1][evalIterNum] = prec
                # Recall
                rec = CORRECT[tag] / GOLD[tag]
                evalStats[tagNum][2][evalIterNum] = rec
                # F1
                f1 = (2 * prec * rec) / (prec + rec)
                evalStats[tagNum][3][evalIterNum] = f1

            print (evalIterNum+1)

            if evalIterNum == (EVAL_NUM -1):
                # Save stats in a pickle
                with open('eval_stats.pickle', 'wb') as handler:
                    pickle.dump(evalStats, handler, protocol=pickle.HIGHEST_PROTOCOL)
                # Print stats
                print "------------\nEvaluation Stats: (Precision, Recall, F1):"
                outFile.write(
                    "------------\nEvaluation Stats: (Precision, Recall, F1):\n")
                for tagNum in range(NUM_ENTITIES):
                    tag = int2tags[tagNum]
                    # finalIndex = np.argmax(evalStats[tagNum][0])
                    # finalCorrect = evalStats[tagNum][0][finalIndex]
                    # finalPrec = evalStats[tagNum][1][finalIndex]
                    # finalRec = evalStats[tagNum][2][finalIndex]
                    # finalF1 = evalStats[tagNum][3][finalIndex]

                    finalCorrectMean, finalCorrectStd = top5_mean_std(evalStats[tagNum][0])
                    finalPrecMean, finalPrecStd = top5_mean_std(evalStats[tagNum][1])
                    finalRecMean, finalRecStd = top5_mean_std(evalStats[tagNum][2])
                    finalF1Mean, finalF1Std = top5_mean_std(evalStats[tagNum][3])

                    print tag, finalPrecMean, finalRecMean, finalF1Mean, "########", finalCorrectMean, PRED[
                        tag], GOLD[tag], "###-std-dev-###", finalF1Std
                    outFile.write(' '.join(
                        [str(tag), str(finalPrecMean), str(finalRecMean), str(finalF1Mean)]) + '\n')
            # print "StepCnt (total, average):", stepCnt, float(stepCnt) / len(
            #     articles)
            # outFile.write("StepCnt (total, average): " + str(stepCnt) + ' ' +
            #               str(float(stepCnt) / len(articles)) + '\n')
            ...
            #                    '\n')
            # outFile2.write("CHANGES: " + str(CHANGES) + ' ' + str(
            #     float(CHANGES) / len(articles)) + "\n")
            # outFile2.write("STAT_POSITIVE, STAT_NEGATIVE " + str(STAT_POSITIVE)
            #                + ', ' + str(STAT_NEGATIVE) + '\n')

            #for analysis
            # pdb.set_trace()

            evalIterNum += 1  # Increment
            ...
            if arguments.saveEntities:
                pickle.dump(
                    [TRAIN_ENTITIES, TRAIN_CONFIDENCES, TRAIN_COSINE_SIM],
                    open("train2.entities", "wb"))
                pickle.dump([TEST_ENTITIES, TEST_CONFIDENCES, TEST_COSINE_SIM],
                            open("test2.entities", "wb"))
                return
            ...
            if arguments.delayedReward == 'True' and terminal == 'false':
                reward = 0
        ...
        if message != "evalStart" and message != "evalEnd" and message != "evalInit":
            ...
            if evalMode and articleNum <= len(articles) and terminal == 'true':
                if arguments.oracle:
                    env.oracleEvaluate(env.goldEntities, ENTITIES[env.indx],
                                       CONFIDENCES[env.indx])
                else:
                    env.evaluateArticle(env.bestEntities.values(),
                                        env.goldEntities,
                                        arguments.shooterLenientEval,
                                        arguments.shooterLastName, evalOutFile)

                stepCnt += sum(env.stepNum)

                #stat sign
                vals = env.calculateStatSign(env.originalEntities,
                                             env.bestEntities.values())
                ...
                for entityNum in [0, 1, 2, 3]:
                    if ANALYSIS and evalMode and env.bestEntities.values()[
                            entityNum].lower() != env.originalEntities[
                                entityNum].lower() and reward > 0:
                        CHANGES += 1
                        try:
                            print "ENTITY:", entityNum
                            print "Entities:", 'best', env.bestEntities.values(
                            )[entityNum], 'orig', env.originalEntities[
                                entityNum], 'gold', env.goldEntities[entityNum]
                            print ' '.join(originalArticle)
                            print "$$$$$$$$$$$$$$$$$$$$$$$$$$$$"
                            print ' '.join(newArticles[env.bestIndex[0]][
                                env.bestIndex[1]])
                            print "----------------------------"
                        except:
                            pass
            ...
            outMsg = str(newstate) + '    ' + str(reward) + '    ' + terminal
...
def main(args):
    ...
    trained_model = pickle.load(open(arguments.modelFile, "rb"))
    ...
    train_articles, train_titles, train_identifiers, train_downloaded_articles, TRAIN_ENTITIES, TRAIN_CONFIDENCES, TRAIN_COSINE_SIM, CONTEXT1, CONTEXT2 = pickle.load(
        open(arguments.trainEntities, "rb"))

    if arguments.contextType == 1:
        TRAIN_CONTEXT = CONTEXT1
    else:
        TRAIN_CONTEXT = CONTEXT2

    CONTEXT_TYPE = arguments.contextType
    ...
    test_articles, test_titles, test_identifiers, test_downloaded_articles, TEST_ENTITIES, TEST_CONFIDENCES, TEST_COSINE_SIM, CONTEXT1, CONTEXT2 = pickle.load(
        open(arguments.testEntities, "rb"))
    ...
    if not arguments.baselineEval and not arguments.thresholdEval and not arguments.confEval:
        ENTITIES = TRAIN_ENTITIES
        CONFIDENCES = TRAIN_CONFIDENCES
        COSINE_SIM = TRAIN_COSINE_SIM
        CONTEXT = TRAIN_CONTEXT
    ...
    if arguments.baselineEval:
        baselineEval(articles, identifiers, arguments)
        return
    elif arguments.thresholdEval:
        thresholdEval(articles, downloaded_articles, identifiers, arguments)
        return
    elif arguments.confEval:
        confEval(articles, downloaded_articles, identifiers, arguments)
        return
    elif arguments.classifierEval:
        print arguments.trainEntities
        print arguments.testEntities

        m = "TEST"
        split_index = 292
        if m == "DEV":
            CLS_TEST_ENTITIES = splitDict(TRAIN_ENTITIES, split_index,
                                          len(TRAIN_ENTITIES))
            CLS_TEST_CONFIDENCES = splitDict(TRAIN_CONFIDENCES, split_index,
                                             len(TRAIN_ENTITIES))
            CLS_TEST_COSINE_SIM = splitDict(TRAIN_COSINE_SIM, split_index,
                                            len(TRAIN_ENTITIES))
            CLS_TEST_CONTEXT = splitDict(TRAIN_CONTEXT, split_index,
                                         len(TRAIN_ENTITIES))
            CLS_test_identifiers = splitDict(train_identifiers, split_index,
                                             len(TRAIN_ENTITIES))

            CLS_TRAIN_ENTITIES = splitDict(TRAIN_ENTITIES, 0, split_index)
            CLS_TRAIN_CONFIDENCES = splitDict(TRAIN_CONFIDENCES, 0,
                                              split_index)
            CLS_TRAIN_COSINE_SIM = splitDict(TRAIN_COSINE_SIM, 0, split_index)
            CLS_TRAIN_CONTEXT = splitDict(TRAIN_CONTEXT, 0, split_index)

            CLS_train_identifiers = splitDict(train_identifiers, 0,
                                              split_index)
        elif m == "TEST":
        ...
        baseline.trainAndEval(CLS_train_identifiers, CLS_test_identifiers,
                              arguments.entity, COUNT_ZERO)
        return

    articleIdx = []
    a = np.arange(len(articles))
    for i in range(PARALLEL_SIZE):
        articleIdx.append(a)
        np.random.shuffle(articleIdx[i])

    outFile = open(arguments.outFile, 'w', 0)  #unbuffered
    outFile.write(str(arguments) + "\n")

    outFile2 = open(arguments.outFile + '.2', 'w', 0)  #for analysis
    outFile2.write(str(arguments) + "\n")

    evalOutFile = None
    if arguments.evalOutFile != '':
        evalOutFile = open(arguments.evalOutFile, 'w')

    processes = []

    for i in range(PARALLEL_SIZE):
        portNum = PORT + i
        p = Process(target=process, args=(portNum, articleIdx[i], arguments))
        processes.append(p)
        p.start()
...
if __name__ == '__main__':
    ...
    argparser.add_argument(
        "--port", type=int, default=5050, help="port for server")
    argparser.add_argument("--trainFile", type=str, help="training File")
    argparser.add_argument(
        "--testFile", type=str, default="", help="Testing File")
    ...
    argparser.add_argument("--modelFile", type=str, help="Model File")
    ...
    argparser.add_argument(
        "--oracle", type=bool, default=False, help="Evaluate using oracle")
...
#sample
#python server2.py --trainEntities consolidated/train+context.5.p --testEntities consolidated/dev+test+context.5.p --outFile outputs/tmp2.out --modelFile trained_model2.p --entity 4 --aggregate always --shooterLenientEval True --delayedReward False --contextType 2
